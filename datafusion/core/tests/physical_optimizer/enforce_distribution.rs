...
#[test]
fn range_date_trunc_locality_survives_aggregate_combination() -> Result<()> {
    ...
    let timestamp = col("ts", &schema)?;
    let range = Partitioning::Range(RangePartitioning::try_new(
        [PhysicalSortExpr::new_default(Arc::clone(&timestamp))].into(),
    ...
    let scan = FileScanConfigBuilder::new(
        ObjectStoreUrl::parse("test:///")?,
        Arc::new(ParquetSource::new(Arc::clone(&schema))),
    )
    .with_file_groups(vec![
        FileGroup::new(vec![PartitionedFile::new("p0", 100)]),
        FileGroup::new(vec![PartitionedFile::new("p1", 100)]),
    ])
    .with_output_partitioning(Some(range))
    .build();
    let source: Arc<dyn ExecutionPlan> = DataSourceExec::from_data_source(scan);
    let bucket: Arc<dyn PhysicalExpr> = Arc::new(ScalarFunctionExpr::new(
        "date_trunc",
        datafusion_functions::datetime::date_trunc(),
        vec![lit("hour"), timestamp],
        Field::new("bucket", timestamp_type, true).into(),
        Arc::new(ConfigOptions::default()),
    ));
    let group_by =
        PhysicalGroupBy::new_single(vec![(Arc::clone(&bucket), "bucket".into())]);
    ...
    let final_plan: Arc<dyn ExecutionPlan> = Arc::new(AggregateExec::try_new(
    ...
    let enforced = EnsureRequirements::new().optimize(final_plan, &config)?;
    let plan_text = displayable(enforced.as_ref()).indent(true).to_string();
    assert!(plan_text.contains("mode=FinalPartitioned"), "{plan_text}");
    assert!(plan_text.contains("mode=Partial"), "{plan_text}");
    assert!(!plan_text.contains("RepartitionExec"), "{plan_text}");
    SanityCheckPlan::new().optimize(Arc::clone(&enforced), &config)?;

    let combined = CombinePartialFinalAggregate::new().optimize(enforced, &config)?;
    let combined_text = displayable(combined.as_ref()).indent(true).to_string();
    assert!(
        combined_text.contains("mode=SinglePartitioned"),
        "{combined_text}"
    );
    let reenforced = EnsureRequirements::new().optimize(combined, &config)?;
    assert_eq!(
        displayable(reenforced.as_ref()).indent(true).to_string(),
        combined_text
    );
    SanityCheckPlan::new().optimize(reenforced, &config)?;

    // Group locality does not establish a join-compatible partition layout.
    let join = crate::physical_optimizer::test_utils::hash_join_exec(
        Arc::clone(&source),
        source,
        vec![(Arc::clone(&bucket), bucket)],
        None,
        &JoinType::Inner,
    )?;
    assert!(
    ...
    let enforced_join = EnsureRequirements::new().optimize(join, &config)?;
    assert!(
        displayable(enforced_join.as_ref())
            .indent(true)
            .to_string()
            .contains("partitioning=Hash")
    );
    SanityCheckPlan::new().optimize(enforced_join, &config)?;
    Ok(())
}
